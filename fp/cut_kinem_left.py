...
class DemoNode(Node):
    ...
    def __init__(self, name, rate):
        ...
        self.chain_larm = KinematicChain(self, 'utorso', 'l_hand', ['l_arm_shz', 'l_arm_shx',
                                                                    'l_arm_ely', 'l_arm_elx',
                                                                    'l_arm_wry', 'l_arm_wrx', 'l_arm_wry2'])
        ...
        (self.p0, self.R0, _, _) = self.chain_larm.fkin(self.q0_larm)                               #  np.array( [4., 2., 4.]).reshape((-1,1) )
        print("R0", self.R0)
        #self.R0 = Reye()

        self.q_larm = self.q0_larm
        self.pd = self.p0 
        self.Rd = self.R0
    ...
    def update(self):
        ...
        pf, Rf, _, _ = self.chain_larm.fkin(np.array([-45., -30., 0., 30., 0., 0., 0.]).reshape((-1,1))) 

        pd_l_f = np.array([0.1158, 0.71051, 1.21365-0.862]).reshape(-1,1) # Final desired position for left arm 
        
        
       # print()
        pd, vd = goto(self.t, 5., self.p0, pf)
        print(pd)
        #pd = pf # pxyz(0,0,0)
        #vd = pxyz(0,0,0)
        # pd = pxyz(0.128 * np.sin(1*self.t), 0.1 * np.sin(1*self.t) + 0.6, 0.428)
        # vd = pxyz(0, 0.1 * np.cos(1*self.t), 0)

        Rd = self.R0
        wd = np.array([0,0,0]).reshape(3,1)

      
        qlast_larm = self.q_larm 
        pdlast = self.pd
        Rdlast = self.Rd
        (ptip, Rtip, Jv, Jw) = self.chain_larm.fkin(qlast_larm) 
        ...
        gamma = 0.01
        Jinv = np.linalg.pinv(J)  # J.T @ np.linalg.pinv(J @ J.T + gamma**2 * np.eye(6))  # np.linalg.pinv(J) #  
        
        # lam_s = 20
        # qdot_back = lam_s * (-q[16:19])
        # qdot_back_full = np.vstack((qdot_back, np.zeros((7,1))))
        # print(qdot_back_full.shape)

        qdot_larm = Jinv @ (x_dot + self.lam * error)  # + (np.eye(10) - Jinv @ J) @ qdot_back_full
        q_larm = qlast_larm + qdot_larm * self.dt 
        self.q_larm = q_larm 
        self.pd = pd 
        self.Rd = Rd
